...
*   **Discord Webhook (optional)**: Sends jail and unjail events to a Discord webhook. If BanHammer is installed, Jail Mod will reuse BanHammer's webhook automatically unless you set one in Jail Mod's config.
...
*   **`discord_webhook_url`**: Optional Discord webhook used for jail/un-jail embeds. Leave empty to auto-use BanHammer's webhook when BanHammer is installed.
